...
class BertPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        ...
        if 'QT' in mode:
            ### QT
            self.dense = nn.Linear(config.hidden_size + 9, config.hidden_size + 9)
            ### QT
        else:
            self.dense = nn.Linear(config.hidden_size, config.hidden_size) #ori
        ...
        if 'QT' in mode:
            ### QT
            self.LayerNorm = BertLayerNorm(config.hidden_size + 9, eps=1e-12)
            ### QT
        else:
            self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12) #ori
