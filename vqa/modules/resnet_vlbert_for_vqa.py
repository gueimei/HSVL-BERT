...
class ResNetVLBERT(Module):
    def __init__(self, config):
        ...
        self.vlbert = VisualLinguisticBert(config.NETWORK.VLBERT,
                                         language_pretrained_model_path=language_pretrained_model_path) ##lang
        ...
        if config.NETWORK.CLASSIFIER_TYPE == "2fc":
            ...
            if 'QT' in mode:
                linear = nn.Linear(config.NETWORK.VLBERT.hidden_size + 9, config.DATASET.ANSWER_VOCAB_SIZE)
            else:
            ...
            for k, v in language_pretrained.items():
                if k.startswith('cls.predictions.transform.'):
                    ...
                    if 'QT' in mode:
                        ...
                        mlm_transform_state_dict[k_] = v_
                        ###QT
                    else:
                        mlm_transform_state_dict[k_] = v #ori
    ...
    def train_forward(self,
        ...
        #print('phrase:', phrase)
        #print('phrase_weight:', phrase_weight)
        #print('label:')
        #labels = []
        #labels.extend(label.argmax(dim=1).detach().cpu().tolist())
        #print(labels)
        #print(djoidjop)
        ...
        if 'QT' in mode:
            ###QT
            #connect Question type
            Q_type = Q_type.float()
            hm = torch.cat((hm, Q_type), 1)
            ###QT
