...
def test_net(args, config, ckpt_path=None, save_path=None, save_name=None):
    ...
    if args.split == 'val2014':
        print('------------- ANN --------------')
        model.eval()
        metrics_list = [vqa_metrics.SoftAccuracy(allreduce=False, num_replicas=1)]
        metrics = CompositeEvalMetric()
        for child_metric in metrics_list:
            metrics.add(child_metric)
        #testMonitor = ValidationMonitor(do_validation, test_loader, metrics, host_metric_name='SoftAcc', label_index_in_batch=-1)
        do_validation(model, test_loader, metrics, label_index_in_batch = -1)
    else:
        q_ids = []
        answer_ids = []
        model.eval()
        ...
        result_json_path = os.path.join(save_path, '{}_vqa2_{}.json'.format(cfg_name if save_name is None else save_name,config.DATASET.TEST_IMAGE_SET))
    ...
    #result = [{'question_id': q_id, 'answer': test_dataset.answer_vocab[a_id]} for q_id, a_id in zip(q_ids, answer_ids)]
    result = [{'answer': test_dataset.answer_vocab[a_id], 'question_id': q_id} for q_id, a_id in zip(q_ids, answer_ids)]
