...
def train_net(args, config):
    ...
    if config.NETWORK.PARTIAL_PRETRAIN != "":
        ...
        smart_partial_load_model_state_dict(model, pretrain_state_dict, config.module_mode)
    ...
    if config.NETWORK.CLASSIFIER_PRETRAINED:
        ...
        for name, module in model.named_modules():
            ...
            if name.endswith('final_mlp'):
                if 'QT' in mode:
                    ###QT
                    weight_tensor = answers_word_embed_tensor.to(device=module[-1].weight.data.device)
                    none_tensor = torch.FloatTensor(3129, 9).cuda()
                    none_tensor = torch.nn.init.xavier_uniform_(none_tensor)
                    module[-1].weight.data = torch.cat((weight_tensor, none_tensor), 1)
                    ###QT
                else:
                    #ori
    ...
    train(model, optimizer, lr_scheduler, train_loader, train_sampler, train_metrics,
          ...
          ##not sure
          gradient_accumulate_steps=config.TRAIN.GRAD_ACCUMULATE_STEPS)####enhance step
